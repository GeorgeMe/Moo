...
import com.is.moo.utils.GUIUtils;
import com.is.moo.utils.TransitionUtils;
import com.is.moo.custom.listeners.AnimatorAdapter;
import com.is.moo.custom.listeners.TransitionAdapter;
import com.is.moo.custom.views.ObservableScrollView;
import com.is.moo.custom.views.ScrollViewListener;
import com.is.moo.entities.Review;
...
/*        if (mIsTablet && mMovieImageView != null) {
            Picasso.with(this).load(url).fit().centerCrop().into(mMovieImageView);
        }*/
    }
...
/*
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP)
            GUIUtils.showViewByRevealEffect(mConfirmationContainer,mFabButton, GUIUtils.getWindowWidth(this));
        else
            GUIUtils.startScaleAnimationFromPivot((int) mFabButton.getX(),(int) mFabButton.getY(),mConfirmationContainer, null);
*/
    ...
    /**  fab点击后的动画 Lollipop & higher versions
    ...
    public void setHeadersTitlColors(Swatch swatch) {
        if (swatch != null) {
            ...
            mTitle.setBackgroundColor(mBrightSwatch.getRgb());
            mReviewsAuthorColor = swatch.getRgb();
            mMovieInfoTextViews.get(CONFIRMATION).setTextColor(swatch.getRgb());
            GUIUtils.tintAndSetCompoundDrawable(this, R.drawable.ic_domain_white_24dp,swatch.getRgb(), mMovieInfoTextViews.get(HOMEPAGE));
            GUIUtils.tintAndSetCompoundDrawable(this, R.drawable.ic_public_white_24dp,swatch.getRgb(), mMovieInfoTextViews.get(COMPANY));
            ButterKnife.apply(movieHeaders, GUIUtils.setter,swatch.getRgb());
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
               // Drawable drawable = mConfirmationView.getDrawable();
               // drawable.setColorFilter(swatch.getRgb(), PorterDuff.Mode.MULTIPLY);
            } else {
                //mConfirmationView.setColorFilter(swatch.getRgb(),PorterDuff.Mode.MULTIPLY);
            }
